...
static INLINE void setpageptr(int s, uint32 A, uint8 *p)
{
 uint32 AB=A>>11;
 int x;
 fprintf(stderr, "cart.c in setpageptr: s %d, A %lx, p %lx\n", s, A, p); 
...
void SetupCartPRGMapping(int chip, uint8 *p, uint32 size, int ram)
{
 ...
 fprintf(stderr, "cart.c in SetupCartPRGMapping: chip %d, p %lx, size %ld so PRGmask8[%d] = %d\n", 
	 chip, p, size, chip, (size>>13)-1);
...
{
  // static int limit = 0;
  //  fprintf(stderr, "cart.c in CartBR: %ld %ld %02x (%d)\n", A, A>>11, Page[A>>11][A], Page[A>>11][A]);
  // if (++limit >= 2) exit(1);
 return Page[A>>11][A];
}
