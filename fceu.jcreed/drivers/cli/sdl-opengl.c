...
{
 if(!HiBuffer)
  glColorTableEXT(GL_TEXTURE_2D,GL_RGB,JCREED_SIZE,GL_RGBA,GL_UNSIGNED_BYTE,data);
}
...
void BlitOpenGL(uint8 *buf)
{
 ...
 if(HiBuffer)
 {
  Blit8ToHigh(buf,HiBuffer,JCREED_SIZE,JCREED_SSIZE,JCREED_SIZE*4,1,1); 
  glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA8,JCREED_SIZE,JCREED_SIZE, 0, GL_RGBA,GL_UNSIGNED_BYTE,
			HiBuffer);
 }
 else
 {
  glPixelStorei(GL_UNPACK_ROW_LENGTH, 272);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_COLOR_INDEX8_EXT, JCREED_SIZE, JCREED_SIZE, 0,
 ...
 glBegin(GL_QUADS);
 glTexCoord2f(1.0f*left/JCREED_SIZE, 1.0f*bottom/JCREED_SIZE);	// Bottom left of our picture.
 glVertex2f(-1.0f, -1.0f);	// Bottom left of target.

 glTexCoord2f(1.0f*right/JCREED_SIZE, 1.0f*bottom/JCREED_SIZE);	// Bottom right of our picture.
 glVertex2f( 1.0f, -1.0f);	// Bottom right of target.

 glTexCoord2f(1.0f*right/JCREED_SIZE, 1.0f*top/JCREED_SIZE);	// Top right of our picture.
 glVertex2f( 1.0f,  1.0f);	// Top right of target.

 glTexCoord2f(1.0f*left/JCREED_SIZE, 1.0f*top/JCREED_SIZE);	// Top left of our picture.
		...
		int stretchx, int stretchy, SDL_Surface *screen)
{
 ...
 if(!strstr(extensions,"GL_EXT_paletted_texture"))
 {
  ...
  HiBuffer=malloc(4*JCREED_SIZE*JCREED_SIZE);
  memset(HiBuffer,0x00,4*JCREED_SIZE*JCREED_SIZE);
  #ifndef LSB_FIRST
 ...
 if(efx&1)
 {
  ...
  scanlines=1;
  buf=malloc(JCREED_SIZE*512*4);

  for(y=0;y<512;y++)
  for(x=0;x<JCREED_SIZE;x++)
  {
   buf[y*JCREED_SIZE*4+x*4]=0;
   buf[y*JCREED_SIZE*4+x*4+1]=0;
   buf[y*JCREED_SIZE*4+x*4+2]=0;
   buf[y*JCREED_SIZE*4+x*4+3]=(y&1)?0x00:0xFF; //?0xa0:0xFF; // <-- Pretty
   //buf[y*JCREED_SIZE+x]=(y&1)?0x00:0xFF;
  }
  glBindTexture(GL_TEXTURE_2D, textures[1]);
  ...
  //glTexImage2D(GL_TEXTURE_2D, 0, GL_COLOR_INDEX8_EXT, JCREED_SIZE, 512, 0,
  //              GL_COLOR_INDEX,GL_UNSIGNED_BYTE,buf);
  
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, JCREED_SIZE, 512, 0,
                GL_RGBA,GL_UNSIGNED_BYTE,buf);
  //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, JCREED_SIZE, 512, 0, GL_ALPHA8, GL_UNSIGNED_BYTE,buf);
  free(buf);
 }
 glBindTexture(GL_TEXTURE_2D, textures[0]);
