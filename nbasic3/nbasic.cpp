...
CodeTree* doArith(CodeTree* op, CodeTree* lhs, CodeTree* rhs){
  ...
  CodeTree *tmp =doLoadA(lhs,"error in nbasic arithmetic expressions (doLoadA).  contact mrl@andrew.cmu.edu");
  result->text=tmp->text;
  delete tmp;
  ...
  case '[':
    ...
    break;
  default:
    yyerror("error in nbasic arithmetic expressions (combine).  contact mrl@andrew.cmu.edu");
    break;
  }
