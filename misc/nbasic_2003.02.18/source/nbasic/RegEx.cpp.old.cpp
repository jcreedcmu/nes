...
int RegEx::Matches(const char* s)
{
	int stringlen = strlen(s);
	if (plainstring)
	{
...
bool RegEx::SetContains(const char* set, char c)
{
...
int RegEx::WhereInSet(const char* set, char c)
{
	int setlen = strlen(set);
	for(int i=0; i<setlen; i++)
		if (c==set[i]) return i;
...
char RegEx::ParseChar(const char* s, int &i)
{
	if(s[i] == '\'' && s[i+2] == '\'')
	{
		char c = s[i+1];
